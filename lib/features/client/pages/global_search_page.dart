...
class _GlobalSearchPageState extends ConsumerState<GlobalSearchPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final isLoading =
        leadsAsync.isLoading || clientsAsync.isLoading || tasksAsync.isLoading;
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            // ── Body ───────────────────────────────────────────────────────
            Expanded(child: _buildBody(context, results, isLoading)),
          ],
        ),
      ),
    );
      ...
      subtitle: Row(
        children: [
          Icon(
            Symbols.schedule,
            size: 12,
            color: isOverdue ? AppColors.danger : AppColors.textLight,
          ),
          const SizedBox(width: 4),
          Text(
            dateLabel,
            style: TextStyle(
              color: isOverdue ? AppColors.danger : AppColors.textMid,
              fontSize: 12,
            ),
          ),
          if (task.isDone) ...[
