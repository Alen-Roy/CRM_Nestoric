...
class CustomBottomNavBar extends StatelessWidget {
  ...
  static const List<_NavItem> _items = [
    ...
    _NavItem(icon: Symbols.leaderboard, activeIcon: Symbols.leaderboard, label: 'Leads'),
    _NavItem(icon: Symbols.person,      activeIcon: Symbols.person,      label: 'Clients'),
    _NavItem(icon: Symbols.task_alt,    activeIcon: Symbols.task_alt,    label: 'Tasks'),
    _NavItem(icon: Symbols.description, activeIcon: Symbols.description, label: 'Reports'),
  ];
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: AppColors.cardDark.withValues(alpha: 0.4), blurRadius: 24, offset: const Offset(0, 8)),
      ...
      child: Row(
        ...
        children: List.generate(
          _items.length,
          (i) => _NavBarItem(item: _items[i], isActive: currentIndex == i, onTap: () => onTap(i)),
        ),
      ),
    );
  }
